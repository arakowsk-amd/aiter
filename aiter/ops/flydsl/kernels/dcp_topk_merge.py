...
def build_dcp_topk_merge_module(n_cand, k_loc, topk_tokens, page_size, world_size):
    ...
    def select_kernel(
        ...
        # once. The four radix passes, the prior-equal sweep and the emit loop
        # all re-scan these same values; reading them from global each time cost
        # 5 x n_cand loads whose latency dominated the kernel (the data is only
        # 64 KB per row -- this was never bandwidth-bound).
    ...
    # Fixed launcher signature: 8 tensors
    @flyc.jit
    def launch(
        gathered_scores: fx.Tensor,
        local_idx: fx.Tensor,
